...
ssr_fft_fp_rotate_2::ssr_fft_fp_rotate_2( int (&fft_phrot_buf_i)[2048] )
  : fft_phrot_buf(fft_phrot_buf_i)
{
...
void ssr_fft_fp_rotate_2::fp_elem_mul (cfloat * restrict x1, cfloat * restrict x2,
...
inline __attribute__((always_inline))
void ssr_fft_fp_rotate_2::fp_dds(int * rom)
{
...
// ------------------------------------------------------------
// Run: idx +1 every 64 loops from 0 upto 1023
// ------------------------------------------------------------

void ssr_fft_fp_rotate_2::run( input_buffer_1d<int,1024,0>& __restrict x0,
                             input_buffer_1d<int,1024,0>& __restrict x1,
                             input_buffer_1d<int, 48, 0>& __restrict tw,
                             output_buffer_1d<int,1024>& __restrict y0,
                             output_buffer_1d<int,1024>& __restrict y1 )
{
  // printf("Run fp_dds for %d;\n", idx);
  if (idxloop==0) fp_dds((int * restrict) tw.data());
  idx = (idx + (idxloop == 0)) & 1023;
