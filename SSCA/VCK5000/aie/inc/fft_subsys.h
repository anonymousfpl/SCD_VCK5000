...
class ssr_win_fft_fp_16x64pt_a: public graph {
public:
  kernel core;
  port<input>  in[2];
  port<output> out;

  ssr_win_fft_fp_16x64pt_a(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_fft_fp_a_2>(std::vector<int>(1024),
                                                 std::vector<int>(1024),
                                                 std::vector<int>{CHEBWIN});
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_fft_fp_a_2.cpp";
...
class ssr_win_fft_cx_fp_16x64pt_a: public graph {
public:
  kernel core;
  port<input>  in[2];
  port<output> out[2];

  ssr_win_fft_cx_fp_16x64pt_a(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_fft_fp_a_xc_2>(std::vector<int>(1024),
                                                 std::vector<int>(1024),
                                                 std::vector<int>{CHEBWIN});
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_fft_fp_a_xc_2.cpp";
...
class ssr_win_fp_16x64pt: public graph {
public:
  kernel core;
  port<input>  in[2];
  port<output> out[2];

  ssr_win_fp_16x64pt(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_ssca_fp_win>(std::vector<int>{CHEBWIN});
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_ssca_fp_win.cpp";
...
class ssr_cx_fp_16x64pt: public graph {
public:
  kernel core;
  port<input>  in[2];
  port<output> out[2];

  ssr_cx_fp_16x64pt(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_ssca_fp_cm>();
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_ssca_fp_cm.cpp";
...
class ssr_dcm_fp_16x64pt: public graph {
public:
  kernel core;
  port<input>  in[1];
  port<output> out[1];

  ssr_dcm_fp_16x64pt(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_ssca_fp_dcm>(std::vector<int>{DCEXP},
                                                  std::vector<int>(128));
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_ssca_fp_dcm.cpp";
...
class ssr_fft_fp_16x64pt_c: public graph {
public:
  kernel core;
  port<input>  in;
  port<output> out;

  ssr_fft_fp_16x64pt_c(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_fft_fp_c_2>(std::vector<int>{TWID2});
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_fft_fp_c_2.cpp";
    // Make connections
    connect<>(in,  core.in[0]);
    connect<>(core.out[0], out);
    location<stack>    (core         )=location<parameter>(core.param[0]);
...
class ssr_fft_fp_16x64pt_c_dcm: public graph {
public:
  kernel core;
  port<input>  in;
  port<output> out;

  ssr_fft_fp_16x64pt_c_dcm(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_fft_fp_c_dcm>(std::vector<int>(2048),
                                                   std::vector<int>{TWID2},
                                                   std::vector<int>{DCEXP},
                                                   std::vector<int>(128));
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_fft_fp_c_dcm.cpp";
    // Make connections
    connect<>(in,  core.in[0]);
    connect<>(core.out[0], out);
    location<stack>    (core         )=location<parameter>(core.param[0]);
...
class ssr_fft_fp_16x64pt_c_dcm_cx: public graph {
public:
  kernel core;
  port<input>  in[2];
  port<output> out[2];

  ssr_fft_fp_16x64pt_c_dcm_cx(void) {
    // Create FFT kernels
    core = kernel::create_object<ssr_fft_fp_c_dcm_cd>(std::vector<int>(2048),
                                                   std::vector<int>{TWID2},
                                                   std::vector<int>{DCEXP},
                                                   std::vector<int>(128));
    runtime<ratio>(core) = 0.8;
    source(core) = "ssr_fft_fp_c_dcm_cd.cpp";
    // Make connections
    connect<>(in[0],  core.in[0]);
    connect<>(in[1],  core.in[1]);
    connect<>(core.out[0], out[0]);
...
// custom added
template<int xoff, int yoff>
class ssr_fft_fp_64pt_x16_graph: public graph {
public:
  ssr_fft_fp_1kpt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c kc;

  port<input>  in[2];
  port<output> out[1];

  ssr_fft_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out, kb.in);
    connect<>(kb.out, kc.in);
    connect<>(kc.out, out[0]);
    ...
    location<stack>(    kc.core)    =  bank(xoff, yoff, 3);
    //
    location<buffer>(kc.core.out[0]) = {address(xoff+1, yoff+1,   0x4000), address(xoff+1, yoff+1, 0x6000)};
...
// custom added
template<int xoff, int yoff>
class ssr_ssca_fft_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fft_fp_16x64pt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c kc;
  ...
  ssr_ssca_fft_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);
    ...
    location<buffer>(ka.core.in[0]) = {address(xoff-1, yoff, 0x4000), address(xoff-1, yoff, 0x6000)};
    location<buffer>(ka.core.in[1]) = {address(xoff-1, yoff, 0x5000), address(xoff-1, yoff, 0x7000)};
    location<parameter>(ka.core.param[1])    =  address(xoff, yoff+1, 0x0000);
    location<parameter>(ka.core.param[2])    =  address(xoff, yoff+1, 0x2000);
    // location<parameter>(ka.core.param[1])    =  {(address(xoff, yoff+1, 0x0000), address(xoff, yoff+1, 0x2000))};
    // location<parameter>(ka.core.param[2])    =  {(address(xoff, yoff+1, 0x1000), address(xoff, yoff+1, 0x3000))};
    location<stack>( ka.core)       =  bank(xoff, yoff+1, 3);
...
// custom added: window+fft+dc
template<int xoff, int yoff>
class ssr_ssca_win_fft_dc_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fft_fp_16x64pt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c_dcm kc;

  port<input>  in[2];
  port<output> out[1];

  ssr_ssca_win_fft_dc_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);
...
// custom added: window+fft+dc+cm
template<int xoff, int yoff>
class ssr_ssca_win_fft_dc_cm_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fft_cx_fp_16x64pt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c_dcm kc;
  ssr_cx_fp_16x64pt kd;

  port<input>  in[2];
  port<output> out[2];

  ssr_ssca_win_fft_dc_cm_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out[0], kb.in);
    connect<>(ka.out[1], kd.in[1]);
    connect<>(kb.out, kc.in);
    connect<>(kc.out, kd.in[0]);
    ...
    location<buffer>(kc.core.in[0]) = {address(xoff+1, yoff+1,   0x0000), address(xoff+1, yoff+1, 0x2000)};
    location<parameter>(kc.core.param[2])    =  address(xoff+1,   yoff,     0x4000);
...
template<int xoff, int yoff>
class ssr_ssca_win_fft_dc_cm_stream_fp_64pt_x16_graph: public graph {
public:
  ...
  ssr_ssca_win_fft_dc_cm_stream_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out[0], kb.in);
    connect<window<128>>(ka.out[1], kd.in[1]);
    connect<>(kb.out, kc.in);
    connect<>(kc.out, kd.in[0]);
    connect<>(kd.out[0],out[0]);
    connect<>(kd.out[1],out[1]);
...
// custom added: window+fft+dc+cx
template<int xoff, int yoff>
class ssr_ssca_win_fft_dc_cx_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fft_cx_fp_16x64pt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c_dcm_cx kc;

  port<input>  in[2];
  port<output> out[2];

  ssr_ssca_win_fft_dc_cx_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out[0], kb.in);
    connect<>(ka.out[1], kc.in[1]);
    connect<>(kb.out, kc.in[0]);
    connect<>(kc.out[0], out[0]);
    connect<>(kc.out[1], out[1]);
    ...
    location<buffer>(kc.core.in[0]) = {address(xoff+1, yoff+1,   0x0000), address(xoff+1, yoff+1, 0x2000)};
    // location<buffer>(kc.core.in[1]) = {address(xoff, yoff,   0x6800), address(xoff, yoff, 0x7000)};
    location<parameter>(kc.core.param[2])    =  address(xoff+1,   yoff,     0x0000);
    location<stack>(    kc.core)    =  bank(xoff, yoff, 3);
    //
    location<buffer>(kc.core.out[0]) = {address(xoff+1, yoff+1,   0x4000), address(xoff+1, yoff+1, 0x6000)};
    location<buffer>(kc.core.out[1]) = {address(xoff+1, yoff+1,   0x5000), address(xoff+1, yoff+1, 0x7000)};
...
// custom added test
template<int xoff, int yoff>
class ssr_ssca_fft_fp_64pt_x16_graph_test: public graph {
public:
  ssr_win_fft_fp_16x64pt_a ka;
  // ssr_fft_fp_1kpt_b kb;
  // ssr_fft_fp_16x64pt_c kc;
  ...
  ssr_ssca_fft_fp_64pt_x16_graph_test(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out, out[0]);
    // connect<>(ka.out, kb.in);
    // connect<>(kb.out, kc.in);
    // connect<>(kc.out, out[0]);
    ...
    // location<parameter>(ka.core.param[2])    =  {(address(xoff, yoff+1, 0x1000), address(xoff, yoff+1, 0x3000))};
    location<stack>( ka.core)       =  bank(xoff, yoff+1, 3);
    location<buffer>(ka.core.out[0]) = {address(xoff, yoff,   0x0000), address(xoff, yoff, 0x2000)};
  };
};
...
// custom added
template<int xoff, int yoff>
class ssr_ssca_win_fft_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fp_16x64pt  kw;
  ssr_fft_fp_1kpt_a ka;
  ssr_fft_fp_1kpt_b kb;
  ssr_fft_fp_16x64pt_c kc;
  ...
  ssr_ssca_win_fft_fp_64pt_x16_graph(void) {
    connect<>(in[0], kw.in[0]);
    connect<>(in[1], kw.in[1]);

    connect<>(kw.out[0], ka.in[0]);
    connect<>(kw.out[1], ka.in[1]);
    ...
    // constraints
    location<kernel>(kw.core) = tile(xoff-1, yoff+1);
    location<kernel>(ka.core) = tile(xoff,   yoff);
    location<kernel>(kb.core) = tile(xoff,   yoff+1);
    location<kernel>(kc.core) = tile(xoff+1, yoff);

    location<buffer>(kw.core.in[0]) = {address(xoff-1, yoff, 0x4000), address(xoff-1, yoff, 0x6000)};
    location<buffer>(kw.core.in[1]) = {address(xoff-1, yoff, 0x5000), address(xoff-1, yoff, 0x7000)};
    location<stack>( kw.core)       =  bank(xoff, yoff+1, 3);
...
// custom added
template<int xoff, int yoff>
class ssr_ssca_win_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fp_16x64pt ka;

  port<input>  in[2];
  port<output> out[2];

  ssr_ssca_win_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);

    connect<>(ka.out[0], out[0]);
    connect<>(ka.out[1], out[1]);

    // constraints
    location<kernel>(ka.core) = tile(xoff,   yoff);

    location<buffer>(ka.core.in[0]) = {address(xoff, yoff, 0x4000), address(xoff, yoff, 0x6000)};
    location<buffer>(ka.core.in[1]) = {address(xoff, yoff, 0x5000), address(xoff, yoff, 0x7000)};
    location<stack>( ka.core)       =  bank(xoff, yoff+1, 3);
    location<buffer>(ka.core.out[0])    =  {address(xoff, yoff+1, 0x0000), address(xoff, yoff+1, 0x2000)};
    location<buffer>(ka.core.out[1])    =  {address(xoff, yoff+1, 0x1000), address(xoff, yoff+1, 0x3000)};
...
// custom added
template<int xoff, int yoff>
class ssr_ssca_dcm_fp_64pt_x16_graph: public graph {
public:
  ssr_dcm_fp_16x64pt ka;

  port<input>  in[1];
  port<output> out[1];

  ssr_ssca_dcm_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    ...
    location<buffer>(ka.core.in[0]) = {address(xoff, yoff, 0x0000), address(xoff, yoff, 0x2000)};
    location<stack>( ka.core)       =  bank(xoff, yoff+1, 3);
    location<buffer>(ka.core.out[0])    =  {address(xoff, yoff, 0x4000), address(xoff, yoff, 0x6000)};
...
// custom added: dc+cx
template<int xoff, int yoff>
class ssr_ssca_dcm_cx_fp_64pt_x16_graph: public graph {
public:
  ssr_fft_fp_16x64pt_c_dcm_cx kc;
  ...
  ssr_ssca_dcm_cx_fp_64pt_x16_graph(void) {
    connect<>(in[0], kc.in[0]);
    connect<>(in[1], kc.in[1]);
    connect<>(kc.out[0], out[0]);
    connect<>(kc.out[1], out[1]);
    ...
    location<buffer>(kc.core.in[0]) = {address(xoff+1, yoff+1,   0x0000), address(xoff+1, yoff+1, 0x2000)};
    location<buffer>(kc.core.in[1]) = {address(xoff, yoff,   0x1000), address(xoff, yoff, 0x1200)};
    location<parameter>(kc.core.param[2])    =  address(xoff+1,   yoff,     0x4000);
    location<stack>(    kc.core)    =  bank(xoff+1, yoff, 3);
    //
    location<buffer>(kc.core.out[0]) = {address(xoff+1, yoff+1,   0x4000), address(xoff+1, yoff+1, 0x6000)};
...
// custom added: conjugate x
template<int xoff, int yoff>
class ssr_ssca_cx_fp_64pt_x16_graph: public graph {
public:
  ssr_win_fft_cx_fp_16x64pt_a ka;
  ...
  ssr_ssca_cx_fp_64pt_x16_graph(void) {
    connect<>(in[0], ka.in[0]);
    connect<>(in[1], ka.in[1]);
    ...
    location<stack>( ka.core)       =  bank(xoff, yoff+1, 3);
    location<buffer>(ka.core.out[0]) = {address(xoff, yoff,   0x0000), address(xoff, yoff, 0x2000)};
    location<buffer>(ka.core.out[1]) = {address(xoff, yoff,   0x6000), address(xoff, yoff, 0x6200)};
